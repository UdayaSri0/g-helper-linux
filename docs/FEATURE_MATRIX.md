...
| Privileged control helper | `rog-privileged` + PolicyKit | Write | Implemented, security-reviewed | API v2 adds a high-level native Aura effect operation to the typed CPU, verified fan, canonical ASUS keyboard LED, and standard battery-threshold methods; no method accepts paths, commands, programs, environments, or raw hardware payloads |
| Privileged helper diagnostics | `rog-daemon` + `rog-cli privileged-status` | Read | Implemented | Distinguishes missing, unreachable, incompatible, PolicyKit unavailable, authorization denied, and backend/category unsupported states; the CLI also verifies installed systemd/D-Bus/PolicyKit/udev files, Aura alias, descriptor, and protocol without hardware writes |
...
| Lighting mode | verified asusd Aura, allow-listed G615JMR target HID, or safe brightness fallback | Read + Write | Backend-dependent | Priority is asusd, native HID, sysfs brightness, unavailable; duplicate native writes are suppressed and active asusd ownership suppresses HID |
| Aura / RGB lighting | exact asusd 6.3.8-6.4.0 DBus contract or native G615JMR target HID | Read + Write | Implemented for narrow contracts; physically unvalidated | Native target is single-zone RGB with Static, Breathe, Rainbow Cycle, Rainbow Wave, and Pulse; it does not claim ARGB, zones, or per-key RGB, and accepted HID writes have no hardware state readback |
| Lighting control centre | GTK4/libadwaita + daemon lighting map | Local staging + Apply | Implemented | Capability-driven effects, validated primary/secondary hex colours and pickers, visual presets, keyboard preview, Current/Pending summaries, Reset, and dirty-only Apply; editing never writes hardware or authenticates |
